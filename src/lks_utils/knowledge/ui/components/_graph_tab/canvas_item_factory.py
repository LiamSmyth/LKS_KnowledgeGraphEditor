...
def _on_instance_dropped(
    ...
    *,
    ...
    if self._current_graph_view is None or not self._canvas._local_node_items:  # noqa: SLF001
        self.set_graph_view(
            new_view,
...
def _on_instances_dropped(self, payload: object) -> None:
    ...
    self._current_graph_view_id = str(new_view.id)
    self._persist_graph_view(new_view)
    if current.nodes and self._canvas._local_node_items:  # noqa: SLF001
...
def _select_canvas_nodes_by_global_ids(self, node_ids: list[str]) -> None:
    ...
    with profile_action(
        ...
        # per item.
        self._canvas.scene.select_items(
            items_to_select,
            additive=False,
...
def _select_canvas_nodes_by_local_ids(self, local_ids: list[str]) -> None:
    ...
    with profile_action(
        ...
        items_to_select = [
            self._canvas._local_node_items.get(local_id)  # noqa: SLF001
...
def _on_canvas_items_moved(self, items: list) -> None:
    """Persist updated proxy positions for moved graph node canvas items."""
    ...
    item_to_local: dict[int, str] = {
        id(it): local_id
        for local_id, it in self._canvas._local_node_items.items()  # noqa: SLF001
    }
    ...
    for item in items:
        ...
        if local_id is None or not isinstance(item, QKnowledgeGraphNodeCanvasItem):
            continue
        b = item.bounds()
...
def _persist_graph_view(self, view: GraphView) -> None:
    ...
    try:
        with self._own_repo_write_scope():
            CanvasIO.save_graph_view(self._session._io, view)  # noqa: SLF001
...
def _persist_graph_view_node_positions(
    ...
    *,
    ...
    view_path = CanvasIO.graph_view_relpath(self._session._io, graph_view_id)  # noqa: SLF001
    ...
    try:
        with self._own_repo_write_scope():
            ...
            return canvas_io.patch_graph_view_node_positions(
                graph_view_id,
...
def _on_remove_selected_proxies(self) -> None:
    ...
    for local_id in selected_local_ids:
        item = self._canvas._local_node_items.pop(local_id)  # noqa: SLF001
    ...
    # _on_canvas_items_moved.
    self._persist_graph_view(new_view)


def install_canvas_item_factory_helpers(cls) -> None:
    ...
    cls._on_canvas_items_moved = _on_canvas_items_moved
...
__all__ = ["install_canvas_item_factory_helpers"]
