"""`ViewportOverlay`: chrome drawn on top of all `CanvasObject`s.

Overlays inherit from `CanvasObject` so they participate in the same
...
class ViewportOverlay(CanvasObject):
    """A `CanvasObject` flagged as chrome.
    ...
    Attributes:
        screen_space: When True, the overlay's :meth:`paint` is given a
            ...
            paints in world space like any other object.
