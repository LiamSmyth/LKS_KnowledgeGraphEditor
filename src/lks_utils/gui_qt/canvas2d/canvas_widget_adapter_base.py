...
from lks_utils.gui_qt.canvas2d.canvas_input_event import CANVAS_ITEM_KEY
...
class CanvasWidgetAdapterBase(CanvasItem, ABC):
    ...
    def handle_input(self, event: CanvasInputEvent) -> bool:
        if event.action.id == CANVAS_ITEM_KEY.id and event.phase in {"press", "release"}:
...
__all__ = ["CanvasWidgetAdapterBase"]
