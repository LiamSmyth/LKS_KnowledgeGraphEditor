...
class FrameTimings:
    ...
    Attributes:
        ...
        items_ms:          All canvas objects combined (timing field name retained).
...
class Canvas2DRenderer:
    ...
    def paint(
        ...
        *,
        ...
        When ``camera.is_minimap`` is True and an object overrides
        :meth:`CanvasObject.paint_minimap`, ``paint_minimap`` is called
        instead of ``paint``.
        ...
        # 2. Objects.
        _tobjects = time.perf_counter()
        for obj in objects:
            # Viewport culling: skip objects that are entirely outside the
            # visible world AABB. Overlays are NEVER culled (handled below).
            if not obj.is_visible(viewport_aabb):
                continue
            painter.save()
            painter.setTransform(world_tf)
            object_opacity = obj.visual_opacity()
            if object_opacity < 0.999:
                painter.setOpacity(max(0.0, min(1.0, object_opacity)))
            try:
                if is_minimap and _has_minimap_override(obj):
                    obj.paint_minimap(ctx)
                else:
                    obj.paint(ctx)
            finally:
                painter.restore()
        _objects_ms = (time.perf_counter() - _tobjects) * 1000.0
...
def _has_minimap_override(obj: CanvasObject) -> bool:
    """Return True if *obj*'s class overrides ``paint_minimap``."""
    return type(obj).paint_minimap is not CanvasObject.paint_minimap
