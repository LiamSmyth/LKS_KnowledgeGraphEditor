...
if TYPE_CHECKING:
    ...
    from lks_utils.gui_qt.canvas2d.overlays.dot_grid_overlay import DotGridOverlay
...
_SHADER_DIR = Path(__file__).parent / "overlays" / "shaders"
...
class Canvas2DGPUOverlayRenderer:
    ...
    def planned_consumed(self, scene: Scene2D, *, layer: OverlayLayer) -> set[int]:
        ...
        for overlay in sorted(scene.overlays(), key=lambda item: item.z_order):
