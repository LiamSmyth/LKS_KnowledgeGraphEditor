...
Shows the union of ``object.bounds()`` for every object in a `Scene2D` plus
...
:meth:`CanvasObject.paint_minimap`. Items that don't override get a
...
Design notes
------------
* Subclasses :class:`CanvasWidgetBase` with a shared ``Scene2D`` and its own
  ``Camera2D(is_minimap=True)`` that auto-fits to content.
* Tracks an external *viewport* ``Camera2D`` (the primary canvas camera).
* Does **not** subclass :class:`Canvas2DWidget` — no object input routing,
  undo history, or selection chrome.
* ``Canvas2DRenderer`` paints items through the minimap camera.
* Click → ``viewport_camera.go_to`` (180 ms); drag inside the viewport
  rect → ``viewport_camera.set_view`` (instantaneous).
...
from lks_utils.gui_qt.canvas2d.render.canvas_renderer import Canvas2DRenderer, FrameTimings
...
from lks_utils.gui_qt.canvas2d.widgets.canvas_widget import CanvasWidgetBase
...
class MinimapWidget(CanvasWidgetBase):
    """Companion overview :class:`CanvasWidgetBase` for a shared `Scene2D`.
    ...
    def __init__(
        ...
        *,
        ...
        self.camera = Camera2D(self, is_minimap=True)
        self.renderer = Canvas2DRenderer()
        ...
        scene.object_added.connect(self._on_scene_changed)
        scene.object_removed.connect(self._on_scene_changed)
        scene.object_changed.connect(self._on_scene_changed)
    ...
    def _padded_aabb(self) -> AABB | None:
        content = self.scene.union_bounds()
        viewport = self.viewport_camera.view().viewport_aabb_world(
    ...
    def _paint_cached_world_layer(self, painter: QPainter, aabb: AABB) -> None:
        ...
        if self._world_cache_dirty or self._world_cache_image is None:
            ...
            self.camera.set_view(mini_view)
            self.renderer.paint(
                self.scene,
                self.camera,
                mini_painter,
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if viewport.contains_point(wp[0], wp[1]):
            ...
            cx, cy = self.viewport_camera.view().center_world
            self._drag_offset_world = (wp[0] - cx, wp[1] - cy)
        else:
            new_view = self.viewport_camera.view().with_center(wp)
            self.viewport_camera.go_to(
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        self.viewport_camera.set_view(
            self.viewport_camera.view().with_center(new_center)
        )
        event.accept()
