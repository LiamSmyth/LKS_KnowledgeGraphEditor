"""`CanvasInputEvent`: a normalised input event delivered to canvas items."""
...
CANVAS_ITEM_WHEEL = Action(
    id="canvas2d.input.item_wheel",
    label="Item wheel",
    category="Canvas",
    description=(
        "Internal: wheel event offered to the canvas item under the cursor "
        "before the canvas handles zoom. Items return True to consume."
    ),
)

# Sentinel action for keyboard events routed to the canvas item under
# the cursor (or active capture item). Adapters can use ``key``/``text``
# to synthesize toolkit-specific key events.
CANVAS_ITEM_KEY = Action(
    id="canvas2d.input.item_key",
    label="Item key",
    category="Canvas",
    description=(
        "Internal: key event offered to canvas items for text-entry "
