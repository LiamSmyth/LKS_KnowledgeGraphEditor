...
class SelectionOverlay(ViewportOverlay):
    """World-space overlay that draws selection borders around every selected item.
    ...
    Args:
        selection: The :class:`~lks_utils.gui_qt.canvas2d.selection_model.SelectionModel`
                   to mirror.
    ...
    def paint(self, ctx: CanvasPaintContext) -> None:
        ...
        items = self._selection.selected_items()
        if not items:
            return
        ...
        for item in items:
            # Skip items that paint their own selection indicator.
            if getattr(item, "manages_own_selection_highlight", False):
                continue
            ...
            # Dashed border around the item's world-space AABB.
