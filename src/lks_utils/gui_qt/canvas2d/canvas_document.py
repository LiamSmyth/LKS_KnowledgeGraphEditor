...
the essential state of a canvas — view, items, overlays, bookmarks,
...
Only items/overlays that implement :meth:`~CanvasItem.to_dict` with a
...
class CanvasDocument:
    ...
    Attributes:
        ...
        view:      The :class:`~lks_utils.gui_qt.canvas2d.view_transform.ViewTransform`
                   at the time of saving, serialised as a plain dict with
                   keys ``center_x``, ``center_y``, ``zoom``, ``rotation_radians``.
        items:     Serialised regular canvas items (non-overlay).
        overlays:  Serialised overlay items.
